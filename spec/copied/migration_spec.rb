...
class SpatialHelper
  def self.geometry_data_types
    ActiveRecord::ConnectionAdapters::PostgreSQLAdapter.geometry_data_types
  end
end
  ...
  describe "creating tables" do
    ...
    (SpatialHelper.geometry_data_types.keys - [:geography]).each do |type|
      it "should create #{type.to_s} columns" do
        ...
        geom_column = @connection.columns(:migrated_geometry_models).detect{|c| c.name == 'geom'}
        ...
        geom_column.srid.should == 0 #DEFAULT_SRID
      end
