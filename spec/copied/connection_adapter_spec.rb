...
describe "Modified PostgreSQLAdapter" do
  ...
  describe "#columns" do
    describe "type" do
      it "should be a regular PostgreSQLColumn if column is a geometry data type" do
        column = PointModel.columns.select{|c| c.name == 'geom'}.first
        column.geometry_type.should == :point
        column.should_not be_geographic
      end

      it "should be a geographic PostgreSQLColumn if column is a geography data type" do
